  ...
  return (
    <div className="max-w-3xl mx-auto glass-card !rounded-2xl p-6 md:p-8">
      <div className="flex items-center gap-2 mb-5">
        ...
        <h2 className="text-lg font-semibold text-surface-100">İleri Seviye Arama</h2>
      </div>
          ...
          {EXAMPLES.map((ex) => (
            <li key={ex.query}>
              ...
              >
                ...
                <p className="text-xs text-surface-400 mt-0.5">{ex.desc}</p>
              </button>
            </li>
          ))}
        </ul>
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5">
          {FIELDS.map((f) => (
            <div key={f.token} className="flex items-baseline gap-2">
              <dt className="text-sm font-mono text-surface-200 shrink-0">{f.token}</dt>
              <dd className="text-xs text-surface-400">{f.desc}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <p className="text-xs text-surface-500 mt-6">
        ...
        <kbd className="px-1.5 py-0.5 text-[10px] rounded bg-surface-700 text-surface-300">/</kbd>{" "}
