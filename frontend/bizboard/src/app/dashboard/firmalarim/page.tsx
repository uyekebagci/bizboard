    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start gap-2">
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-semibold text-[rgb(var(--v2-ink))] flex items-center gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isAdmin && (
            <>
              <button onClick={() => { setCreateFirmGroupId(null); setShowCreateFirm(true); }}
                className="px-3 py-2 rounded-xl bg-[rgb(var(--v2-ink))] text-[rgb(var(--v2-card))] hover:opacity-90 text-xs font-semibold inline-flex items-center gap-1.5">
          ...
          <div className="inline-flex rounded-xl border border-[rgb(var(--v2-border))] overflow-hidden v2-sunken">
            <button onClick={() => setView("list")}
              className={cn("px-2.5 py-2 text-xs font-medium inline-flex items-center gap-1.5",
                view === "list" ? "bg-[rgb(var(--v2-card))] text-[rgb(var(--v2-ink))]" : "text-[rgb(var(--v2-muted))] hover:text-[rgb(var(--v2-ink))]")}>
              <ListIcon size={12} /> Liste
            </button>
            <button onClick={() => setView("group")}
              className={cn("px-2.5 py-2 text-xs font-medium inline-flex items-center gap-1.5 border-l border-[rgb(var(--v2-border))]",
                view === "group" ? "bg-[rgb(var(--v2-card))] text-[rgb(var(--v2-ink))]" : "text-[rgb(var(--v2-muted))] hover:text-[rgb(var(--v2-ink))]")}>
            ...
            return (
              <div key={s.id}
                ...
                className={cn("rounded-2xl border transition-colors",
                  isDragOver ? "border-[rgb(var(--accent))]/60 bg-[rgb(var(--accent))]/5" : "border-[rgb(var(--v2-border))] v2-card")}>
                {/* Header */}
                <div className="px-3 py-2.5 flex items-center gap-2">
                  ...
                  <span
                    className={cn(
                      ...
                      !s.color && "text-[rgb(var(--v2-muted))] border-[rgb(var(--v2-border))]",
                    )}
                    style={s.color ? {
                  ...
                  {isAdmin && !isUngrouped && s.group && (
                    <>
                      ...
                      <button onClick={() => { setCreateFirmGroupId(s.id); setShowCreateFirm(true); }}
                        className="ml-auto px-2 py-1 rounded-md v2-sunken text-[rgb(var(--v2-ink))] text-[10px] font-medium inline-flex items-center gap-1 border border-[rgb(var(--v2-border))]">
...
function FirmCard({
  ...
  return (
    <button onClick={onClick}
      ...
      className={cn(
        "text-left p-3 rounded-xl border border-[rgb(var(--v2-border))] v2-card",
        "hover:border-[rgb(var(--accent))]/40 transition-colors",
...
function EmptyState({ isAdmin, onCreate }: { isAdmin: boolean; onCreate: () => void }) {
  if (!isAdmin) {
    return (
      <div className="py-12 px-6 text-center rounded-2xl border border-[rgb(var(--v2-border))] v2-card">
